  ...
  apply() {
    ...
    if (
      ...
      if (this.validloanamount() && this.validmonth() && this.vaiddoccharge()) {
        ...
        this.capitalPerMonth = (
          Number(this.loanamount) / Number(this.month)
        ).toFixed(2);
        this.interestPerMonth = (
          (Number(rate) / Number(1200)) *
          Number(this.loanamount)
        ).toFixed(2);
        this.totalPerMonth = (
          Number(this.capitalPerMonth) + Number(this.interestPerMonth)
        ).toFixed(2);
  ...
  payadvace() {
    if (this.cusfullname2 && this.paytype && this.advancepay) {
      if (Number(this.advancepay)) {
        if (
          (this.selectedRadio == "2" && this.chequeno) ||
          this.selectedRadio == "1"
        ) {
          //if(this.chequeno){
  ...
  calloan() {
    if (
      ...
      if (this.validloanamount() && this.validmonth() && this.vaiddoccharge()) {
        if (this.checked) {
          this.dicapital = (
            (Number(this.loanamount) + Number(this.doccharge)) /
            Number(this.month)
          ).toFixed(2);
        ...
        this.diinstare = (
          (Number(this.dilaoanamount) * Number(this.dimonthrate)) /
          100
        ).toFixed(2);
        this.ditotal = (
          Number(this.dicapital) + Number(this.diinstare)
        ).toFixed(2);
        this.difulltotal = (Number(this.ditotal) * Number(this.month)).toFixed(
          2
        );
